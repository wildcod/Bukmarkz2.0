...
const Header = ({ auth, logOutUser, t }) => {
    ...
    const scroll = (link) => {
        const element = document.getElementById(`bukmarkz-home-${link.key === 'service' ? 'services' : 'price'}`)
                     ...
                     <ul>
                         {
                             navLinks.map((link => {
                                if(SINGLE_PAGE_ROUTES.includes(link.key)){
                                    return (
                                        <li onClick={() => onNavClick(link)} className={link.link === location?.pathname ? style.active : ''}>
                                                {link.displayName}
                                        </li>
                                    )
                                }else {
                                    return (
                                        <li onClick={onCloseSideBar} className={link.link === location?.pathname ? style.active : ''}>
                                            <Link to={link.link}>
                                                {link.displayName}
                                            </Link>
                                        </li>
                            ...
                            <ul>
                                {
                                    navLinks.map((link => {
                                        if(SINGLE_PAGE_ROUTES.includes(link.key)){
                                            return (
                                                <li onClick={() => onNavClick(link, true)} className={link.link === location?.pathname ? style.active : ''}>
                                                        {link.displayName}
                                                </li>
                                            )
                                        }else {
